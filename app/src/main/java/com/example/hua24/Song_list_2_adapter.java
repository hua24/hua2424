...
public class Song_list_2_adapter extends BaseAdapter {
    public LayoutInflater layoutInflater;
    public Context mycontext;
    public List<String> mylist;
    Map<Integer, Boolean> map=new HashMap<>();//标记checkbox选中状态，防止listview错误显示
    public Song_list_2_adapter(Context context, List<String> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //设置内容
        if(Song_list_2.ckeckbox)
            checkBox.setVisibility(View.VISIBLE);//使checkbox为可见状态
        checkBox.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(checkBox.isChecked()){
                    Song_list_2.picked.add(mylist.get(position));//添加选中的项进入列表
                    map.put(position,true);//标记位置
                }
                else {
                    Song_list_2.picked.remove(mylist.get(position));//移除选中项
